...
export async function generatePaymentsForSubscription(subscriptionId: string) {
  try {
    ...
    const generateUntilDate = subscription.endDate ? new Date(subscription.endDate) : (() => {
    ...
    while (isBefore(nextDueDate, generateUntilDate) && (!subscriptionEndDate || isBefore(nextDueDate, subscriptionEndDate))) {
...
export async function generatePaymentsForAllActiveSubscriptions(userId: string) {
  try {
