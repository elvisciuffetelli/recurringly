...
export default function NotificationBell() {
  ...
  const { notifications, removeNotification, clearAllNotifications, hasUnreadNotifications } = useNotifications()
  ...
  const sendEmailNotifications = async () => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`Email notifications sent! ${result.emailsSent} emails sent successfully.`)
      } else {
  ...
  const testDailyReminders = async () => {
    try {
      ...
      if (response.ok) {
        const result = await response.json()
        alert(`Daily reminder test completed! ${result.summary?.emailsSent || 0} emails sent.`)
      } else {
        const error = await response.json()
